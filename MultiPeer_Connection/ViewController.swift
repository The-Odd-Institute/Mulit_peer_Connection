...
class ViewController: UIViewController, MCSessionDelegate, MCNearbyServiceBrowserDelegate, MCNearbyServiceAdvertiserDelegate, UITableViewDataSource, UITableViewDelegate
{
    ...
    @IBAction func sendAction(_ sender: Any) {
        ...
        if let data = toMessage.data(using: .utf8) {
            do {
                try mySession.send(data, toPeers:  [curCorrespondingPeer], with: .unreliable)
            } catch {
                print("[Error] \(error)")
            }
        }
                    ...
                    invitationHandler: @escaping (Bool, MCSession?) -> Void)
    {
        let alert = UIAlertController(title: "Invitation Recievd",
                                      message: "New invitation from \(peerID.displayName), what?",
                                        preferredStyle: UIAlertControllerStyle.alert)
        
        alert.addAction(UIAlertAction(title: "Accept", style: UIAlertActionStyle.default, handler: { (alert) in
            invitationHandler(true, self.mySession)
        }))
        
        alert.addAction(UIAlertAction(title: "Deny", style: UIAlertActionStyle.cancel, handler: { (alert) in
